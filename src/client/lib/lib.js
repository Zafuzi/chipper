import {rplc8} from "./rpcl8";
import {GLOBAL_LOG_LEVEL} from "./.globals.js";
...
export let AppBody;
export const SetPageTemplate = function(template)
{
    AppBody = rplc8("#AppBody", {content: template});
    document.querySelector("#AppBody").classList.remove("transparent");
};

export const Logger = function(prefix, level)
{
    level = Math.min(GLOBAL_LOG_LEVEL, level);
    return {
        level,
        prefix,
        error()
